...
(function($){"use strict";$(function(){$('#uix-page-builder-visualBuilder-loader, #uix-page-builder-visualBuilder-loader .loader').hide();$(document).on('click','#uix-page-builder-themepreview-btn-close',function(e){e.preventDefault();var oldPo=$('#uix-page-builder-themepreview').css('left').replace('px',''),target=$('.uix-page-builder-gridster-addbtn.visualBuilder, #uix-page-builder-gridster-wrapper.visualBuilder, #uix-page-builder-themepreview, .uix-page-builder-themepreview-btn, .uix-page-builder-themepreview-btn#uix-page-builder-themepreview-btn-close');if(oldPo==0){target.removeClass('active');}else{target.addClass('active');}});$(document).on('mouseenter','.uix-page-builder-visual-mode',function(){var curtitle=$('[name="post_title"]').val().replace(/&/g,'{and}').replace(/ /g,'{space}'),oldhref=$(this).attr('href');if(curtitle.length>0){$(this).attr('href',oldhref+'&post_title='+encodeURI(curtitle))}});$(document).on('mousedown','.uix-page-builder-visual-mode',function(){$(window).unbind();});$(document).on('click','.widget-item-btn',function(e){e.preventDefault();var cur_slug=$(this).data('slug'),cur_rowID=$(this).data('id'),widget_name=$(this).data('name'),ele_target=$(this).data('elements-target'),ele_btn=$('#'+ele_target).find('.add-elements-btn'),cur_defaultvalue=$('#'+ele_target).find('textarea').val();if(ele_btn.css('display')!='none'){ele_btn.after($(this).clone().addClass('used').text(widget_name)).hide();if($('#'+ele_target).find('textarea').length>0){if(cur_defaultvalue.indexOf('uix_pb_section_undefined')>=0){var nv=cur_defaultvalue.replace('uix_pb_section_undefined',cur_slug).replace('uix_pb_undefined',cur_slug+'_temp');$('#'+ele_target).find('textarea').val(nv);gridsterItemSave(cur_rowID);uixPBFormDataSave();}
