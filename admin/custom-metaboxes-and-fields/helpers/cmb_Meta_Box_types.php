...
class cmb_Meta_Box_types {
	...
	public function render_repeatable_field() {
		...
		<p class="add-row">
			<a data-selector="<?php echo $table_id; ?>" class="add-row-button button" href="#"><?php _e( 'Add Row', 'uix-products' ); ?></a>
		</p>
	...
	protected function repeat_row( $class = 'repeat-row' ) {
		...
		<tr class="<?php echo $class; ?>">
			...
			<td class="remove-row">
				<a class="button remove-row-button" href="#"><?php _e( 'Remove', 'uix-products' ); ?></a>
			</td>
		</tr>
	...
	public function taxonomy_radio() {
		...
		if ( ! $terms ) {
			$options .= '<li><label>'. __( 'No terms', 'uix-products' ) .'</label></li>';
		} else {
			foreach ( $terms as $term ) {
				$args = array(
					'value' => $term->slug,
