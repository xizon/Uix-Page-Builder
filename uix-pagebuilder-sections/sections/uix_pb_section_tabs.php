...
for ( $k = 1; $k <= $clone_max; $k++ ) {
	...
	if ( is_array( $item ) && !array_key_exists( '['.$colid.']'.$_uid.'['.$_field.']['.$sid.']', $item ) && $k == 1 ) {
		...
		$list_tabs_item_title .= '<li>'.$uix_pb_tabs_listitem_title.'</li>';
		$list_tabs_item_content .= '
		<div class="uix-pb-spoiler-content">
...
if ( $sid >= 0 && is_admin() ) {
	...
	for ( $i = 2; $i <= $clone_max; $i++ ) {
		...
		if ( is_array( $item ) && array_key_exists( '['.$colid.']'.$uid.'['.$field.']['.$sid.']', $item ) ) {
			...
			$value         =  [
								array(
									'id'       => $uid.UixPageBuilder::fid( $colid, $sid, 'uix_pb_tabs_listitem_title' ),
									'default'  => $item[ '['.$colid.']'.$uid.'[uix_pb_tabs_listitem_title]['.$sid.']' ]
								),
								array(
									'id'       => $uid.UixPageBuilder::fid( $colid, $sid, 'uix_pb_tabs_listitem_con' ),
