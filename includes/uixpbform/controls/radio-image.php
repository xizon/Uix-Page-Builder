<?php
class UixPBFormType_RadioImage  extends UixPBFormCore {
			...
			if ( is_array( $default ) && !empty( $default ) ) {
				...
				if ( is_array( $toggle ) && !empty( $toggle ) ) {
					...
					foreach ( $default as $select_key => $select_value ) {
						...
						$optionlist .= '<span data-value="'.$select_key.'" class="img'.$selected.' '.( !empty( $toggle_trigger_id ) ? 'uixpbform_btn_trigger-toggleswitch_radio' : '' ).'" '.( !empty( $toggle_trigger_id ) ? 'data-targetid="'.$target_id.'"' : '' ).'><img alt="" title="'.esc_attr( $select_key ).'" src="'.esc_url( $select_value ).'"></span>'.PHP_EOL;
						$optionloop ++;
					}	
